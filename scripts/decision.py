...
class Decision(Node):
    ...
    def __init__(self):
        ...
        self.dis_init_fleft = 0.5
        self.dis_init_fright = 0.5
    ...
    def dec_callback(self, msg_data: ObstacleDistance):
        ...
        # initial part
        self.dis_init_fleft = msg_data.init_fleft
        self.dis_init_fright = msg_data.init_fright
    ...
    def timer_callback(self):
        ...
        # initial
        init_fleft_free = self.dis_init_fleft > init_threshold
        init_fright_free = self.dis_init_fright > init_threshold

        self.displacement = sqrt(pow(self.x-self.xref,2)+pow(self.y-self.yref,2))

        if self.dis_front == float("nan") or self.dis_front == 0.0:   # could implement failsafe
            decision_state = 0 # stop the robot if front is not free
            angular_error = 0.0

        elif self.state == "find_wall":
    
            if not init_fleft_free:
                decision_state = 2 # turn left
            elif not init_fright_free:
                decision_state = 3 # turn right
            else:
                decision_state = 1 # straight
            
            if self.displacement > 1.5: 
                decision_state = 1
        ...
        # elif self.displacement > 1.5 or self.at_wall:
            ...
            # alternative p controller implementation with wall following
            ...
            if not front_free:
                ...
                decision_state = 5  # PD wall following

                current_time = self.get_clock().now()
                current_time = current_time.nanoseconds / 1e9
                dt = current_time - self.last_time
                
                error = target_distance - self.dis_right
        ...
        # could potentially replace the above with the following
        # else:
        #     if not init_fleft_free:
        #         decision_state = 2 # turn left
        #     elif not init_fright_free:
        ...
        self.dec_msg.correction = correction
        # Output to terminal
        self.get_logger().info(
            f"Current Decision state: {self.dec_msg.decision_state,self.state,angular_error,correction}",
