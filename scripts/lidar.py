...
class Lidar(Node): 
    ...
    def lidar_callback(self, scan_data: LaserScan): 
        ...
        mid_left_20_deg = scan_data.ranges[0:25]
        mid_right_20_deg = scan_data.ranges[-25:-1] 
        ...
        right = np.array(scan_data.ranges[-135:-1])


        # ranges for the initial exploration to reach the wall
        init_fleft = np.array(scan_data.ranges[0:25])
        init_fright = np.array(scan_data.ranges[-25:-1])
        ...
        initial_fleft_valid_data = init_fleft[(init_fleft != float("inf")) & (init_fleft != 0.0)] 
        if np.shape(initial_fleft_valid_data)[0] > 0: 
            initial_fleft_valid_data_mins = np.sort(initial_fleft_valid_data)[:5]
            initial_fleft_single_point_average = initial_fleft_valid_data_mins.mean()  
        else:
            initial_fleft_single_point_average = float("nan") 

        initial_fright_valid_data = init_fright[(init_fright != float("inf")) & (init_fright != 0.0)] 
        if np.shape(initial_fright_valid_data)[0] > 0: 
            initial_fright_valid_data_mins = np.sort(initial_fright_valid_data)[:5]
            initial_fright_single_point_average = initial_fright_valid_data_mins.mean()  
        else:
            initial_fright_single_point_average = float("nan") 
        ...
        self.lidar_msg.init_fright = float(initial_fright_single_point_average)
        self.lidar_msg.init_fleft = float(initial_fleft_single_point_average)
